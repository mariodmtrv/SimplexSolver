...
public class LaTeXBuilder {
	...
	public String iterationToString(SimplexTable simtable, Pair<Integer,Integer> key) {
		StringBuilder latex = new StringBuilder();
		
		latex.append("\\hline");
		for (int i = 0; i < simtable.getBasisSize(); i++) {
			latex.append(simtable.getBasis().get(i).toString() + "  ");
			latex.append("&" + simtable.getZfunctionCoefficients().get(i).toString());
			
			for(int j = 0; j < simtable.getVarCount(); j++) {
				if(i == key.getFirst() && j == key.getSecond()) {
					// this is the key element  =>  highlight it
					latex.append("&\\mbox{\\fbox{$" + simtable.getSimplexTableItem(i, j) + "$}}");
				}
				else {
